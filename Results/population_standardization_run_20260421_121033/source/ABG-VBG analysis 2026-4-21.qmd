...
mi_unweighted_cat3_fits_abg <- setNames(lapply(cat3_outcomes, function(x) vector("list", imp_n)), cat3_outcomes)
mi_unweighted_cat3_fits_vbg <- setNames(lapply(cat3_outcomes, function(x) vector("list", imp_n)), cat3_outcomes)
...
mi_unweighted_spline_fits_abg <- setNames(lapply(cat3_outcomes, function(x) vector("list", imp_n)), cat3_outcomes)
mi_unweighted_spline_fits_vbg <- setNames(lapply(cat3_outcomes, function(x) vector("list", imp_n)), cat3_outcomes)
...
for (i in seq_len(imp_n)) {
  ...
  # Keep an in-memory cache as well so late chunks do not depend on scratch
  # files surviving the full render. The RDS files are still written because
  # they are useful for diagnostics and post-hoc inspection outside the live
  # notebook session.
  mi_logistic_ps_abg_list[[i]] <- list(
    weights_path = w_path_abg,
    ps_path = ps_path_abg,
    weights_cache = w_abg_i,
    ps_cache = ps_abg_i,
  ...
  for (outcome_var in cat3_outcomes) {
    ...
    mi_unweighted_cat3_fits_abg[[outcome_var]][[i]] <- fit_abg_cat_unweighted$fit
    if (!is.null(fit_abg_cat_unweighted$diag)) {
      cat3_diag_rows[[length(cat3_diag_rows) + 1L]] <- fit_abg_cat_unweighted$diag
    }
    ...
    mi_unweighted_cat3_fits_vbg[[outcome_var]][[i]] <- fit_vbg_cat_unweighted$fit
    if (!is.null(fit_vbg_cat_unweighted$diag)) {
      cat3_diag_rows[[length(cat3_diag_rows) + 1L]] <- fit_vbg_cat_unweighted$diag
    }
  }
  ...
  for (outcome_var in cat3_outcomes) {
    ...
    mi_unweighted_spline_fits_abg[[outcome_var]][[i]] <- spline_abg_mi_unweighted
    if (!is.null(spline_abg_mi_unweighted$diag)) {
      spline_diag_rows[[length(spline_diag_rows) + 1L]] <- spline_abg_mi_unweighted$diag
    }
    ...
    mi_unweighted_spline_fits_vbg[[outcome_var]][[i]] <- spline_vbg_mi_unweighted
    if (!is.null(spline_vbg_mi_unweighted$diag)) {
      spline_diag_rows[[length(spline_diag_rows) + 1L]] <- spline_vbg_mi_unweighted$diag
    }
  }
  ...
  rm(d, fit_abg, fit_vbg, w_abg_i, w_vbg_i, w_unweighted_i, ps_abg_i, ps_vbg_i,
...
# helper: retrieve one cached or on-disk MI weight vector for the requested
# cohort and imputation without rebuilding anything.
get_mi_weight <- function(group, i) {
  ...
  if (!is.null(meta$weights_cache)) {
    return(meta$weights_cache)
  }
  stopifnot(!is.null(meta$weights_path), file.exists(meta$weights_path))
  readRDS(meta$weights_path)
}

# helper: retrieve one cached or on-disk MI propensity-score vector for the
...
get_mi_ps <- function(group, i) {
  ...
  if (!is.null(meta$ps_cache)) {
    return(meta$ps_cache)
  }
...
table_2_weighted_categorical_outcomes <- dplyr::bind_rows(
  ...
  table_2_weighted_probability_panel |>
    ...
    dplyr::select(panel, group, outcome, n_model, events, measure, estimate)
)
...
standardization_sample_cap <- if (IS_PILOT) 1000L else 20000L
...
standardization_source_rows <- function(d, imp_index) {
  d <- d[, intersect(c(adj_core, "has_abg", "has_vbg"), names(d)), drop = FALSE]
  d <- d[stats::complete.cases(d[, adj_core, drop = FALSE]), , drop = FALSE]
  if (nrow(d) <= standardization_sample_cap) return(d)
  set.seed(7100L + imp_index)
  d[sample.int(nrow(d), standardization_sample_cap), , drop = FALSE]
}
...
marginal_predict_compact <- function(fit_compact, source_df, co2_var,
  ...
  purrr::map_dfr(co2_values, function(co2_value) {
    nd <- source_df
    nd[[co2_var]] <- co2_value
    pred <- predict_spline_prob_from_compact(
      fit_compact,
      grid_df = nd,
      outcome_var = outcome_var,
      co2_var = co2_var
    ...
    tibble::tibble(
      co2_value = co2_value,
      p_imp = mean(pred$p, na.rm = TRUE),
      n_standardized = nrow(nd)
    )
  })
}
...
discordance_marginal_standardized_prob_curves <- marginal_imp_curves |>
  dplyr::group_by(outcome, outcome_var, group, co2_scale, strategy, co2_value) |>
  dplyr::summarise(
    p = mean(p_imp, na.rm = TRUE),
    p_LCL = stats::quantile(p_imp, 0.025, na.rm = TRUE, names = FALSE),
    p_UCL = stats::quantile(p_imp, 0.975, na.rm = TRUE, names = FALSE),
    m_used = dplyr::n_distinct(imp),
    n_standardized = stats::median(n_standardized, na.rm = TRUE),
...
if (length(gas_time_candidates) && length(proc_time_candidates)) {
  discordance_timing_status <- tibble::tibble(
    component = "timing_sensitivity",
    status = "available_fields_detected",
    gas_time_fields = paste(gas_time_candidates, collapse = "; "),
    procedure_time_fields = paste(proc_time_candidates, collapse = "; "),
    reason = "candidate_fields_available_manual_review_needed"
  )
  discordance_timing_sensitivity <- tibble::tibble(
    outcome = NA_character_,
    group = NA_character_,
    n_candidate_gas_time_fields = length(gas_time_candidates),
    n_candidate_procedure_time_fields = length(proc_time_candidates),
    status = "candidate_fields_detected",
    reason = "Field naming is not specific enough for an automated before-procedure restriction."
  )
  record_discordance_status(
    "timing_sensitivity",
    "skipped",
    "Candidate timing fields were detected, but no unambiguous gas/procedure timestamp pairs were available.",
  ...
  discordance_timing_status <- tibble::tibble(
    ...
    gas_time_fields = paste(gas_time_candidates, collapse = "; "),
    procedure_time_fields = paste(proc_time_candidates, collapse = "; "),
    reason = "timestamp_fields_unavailable"
  )
  discordance_timing_sensitivity <- tibble::tibble(
    outcome = NA_character_,
    group = NA_character_,
  ...
  record_discordance_status(
    "timing_sensitivity",
    "skipped",
    "Blood-gas/procedure timestamp fields were unavailable.",
    "Results/discordance_timing_status.csv",
    optional = TRUE
  )
}
...
sentinel_selected_n <- sum(discordance_sentinel_procedure_summary$selected %in% TRUE, na.rm = TRUE)
sentinel_inconclusive <- sentinel_selected_n < 2L
sentinel_supported <- !sentinel_inconclusive && any(
  abs(discordance_sentinel_procedure_summary$weighted_abs_diff) > 0.05 |
    discordance_sentinel_procedure_summary$date_missing_abs_diff > 0.05,
  na.rm = TRUE
)
...
discordance_interpretation_summary <- tibble::tibble(
  ...
  key_evidence = c(
    ...
    ifelse(setting_supported, "Within-modality weighted event-rate range exceeded 10 percentage points for at least one setting variable.", "Within-modality setting-specific weighted event-rate ranges were <=10 percentage points."),
    ifelse(capture_supported, "Detected missingness/capture summaries show >5 percentage-point or SMD-scale differences.", "Detected missingness/capture summaries did not show >5-point differences."),
    ifelse(sentinel_inconclusive, "Fewer than two suitable sentinel procedure concepts were selected.", ifelse(sentinel_supported, "At least one sentinel procedure showed >5-point capture or timestamp-completeness difference.", "Selected sentinel procedures did not show >5-point capture/completeness differences.")),
    ...
    ifelse(timing_inconclusive, "Timestamp fields were unavailable or ambiguous.", "Timing fields were available for direct review."),
...
record_discordance_status(
  ...
  "completed",
  "Hypothesis-level interpretation summary was generated.",
  "Results/discordance_interpretation_summary.csv"
)
...
cat(
  ...
  "- Optional sentinel-procedure, timing, first-encounter, dual-tested, and site-specific diagnostics may be marked skipped when required fields or rows are unavailable.\n",
...
expected_diag <- c(
  ...
  "discordance_dual_tested_sensitivity.csv",
  "discordance_first_encounter_sensitivity.csv",
  "discordance_timing_sensitivity.csv", "discordance_timing_status.csv",
  "discordance_interpretation_summary.csv",
  "figs/discordance_probability_standardization_niv_imv.png",
  "figs/discordance_tail_support_niv_imv.png"
)
