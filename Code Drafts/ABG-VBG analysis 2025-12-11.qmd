...
```{r table2-outcome-panel, echo=FALSE, message=FALSE, warning=FALSE}
...
# Combine overall + strata for display (limit to top missing variables)
top_miss <- miss_overall |>
  arrange(desc(pct_missing_overall)) |>
  slice_head(n = 25) |>
  pull(variable)

miss_panel <- miss_by_strata |>
  filter(variable %in% top_miss) |>
  left_join(miss_overall, by = "variable") |>
  arrange(desc(pct_missing_overall), stratum, level)

gt::gt(miss_panel) |>
  gt::tab_header(title = "Missingness by key strata (pre-imputation)") |>
  gt::cols_label(
    variable              = "Variable",
    stratum               = "Stratum",
    level                 = "Level",
    pct_missing           = "% missing in level",
    pct_missing_overall   = "% missing overall"
  ) |>
  gt::fmt_number(columns = c(pct_missing, pct_missing_overall), decimals = 1) |>
  gt::fmt_missing(columns = gt::everything(), missing_text = "—")
...
if (nrow(model_results) > 0) {
  model_results |>
    mutate(variable = factor(variable, levels = vars_to_model)) |>
    arrange(variable, desc(abs(log(OR)))) |>
    head(80) |>  # keep table readable
    gt::gt() |>
    gt::tab_header(title = "Predictors of missingness (logit OR)") |>
    gt::cols_label(
      variable = "Target variable (missingness)",
      term     = "Covariate",
      OR       = "OR",
      LCL      = "2.5%",
      UCL      = "97.5%",
    ...
    gt::fmt_number(columns = c(OR, LCL, UCL), decimals = 2) |>
...
md.pattern(imp$data)                                  # pattern before MI (compact)
```
