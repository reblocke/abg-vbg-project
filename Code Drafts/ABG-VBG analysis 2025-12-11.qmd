...
required_pkgs <- c(
  ...
  "sensitivitymw", "here", "fs", "dagitty", "ggdag"
)
...
# Centralize outputs under Results/, with paths relative to this QMD directory
qmd_dir      <- here::here("Code Drafts")
results_dir  <- here::here("Results")
fig_dir      <- fs::path(results_dir, "figs")
cache_dir    <- fs::path(results_dir, "cache", "abg-vbg-2025-12-11")
fig_path     <- paste0(fs::path_rel(fig_dir, start = qmd_dir), "/")
cache_path   <- paste0(fs::path_rel(cache_dir, start = qmd_dir), "/")
results_path <- function(...) fs::path(results_dir, ...)
fs::dir_create(c(results_dir, fig_dir, cache_dir))
...
.build_model_dag <- function(fml) {
  ...
  if (is.na(vars$response) || length(vars$preds) == 0L) return(NULL)
...
save_model_diagram <- function(name, fml, width = NULL, height = NULL) {
  ...
  p <- ggplot2::ggplot(
    ...
    ggdag::geom_dag_edges(color = "grey40") +
    ggdag::geom_dag_point(size = point_size, color = "black") +
...
register_model_diagram <- function(name, fml, width = 8, height = 6) {
  if (!is.null(model_diagrams[[name]])) return(invisible(model_diagrams[[name]]))
...
#| code-block-title: "Table 2a: crude outcomes by hypercapnia group"
...
rate_panel <- bind_rows(
  subset_data %>%
    ...
    mutate(group = "ABG", hyper_group = as.character(hyper_abg)),
  subset_data %>%
    filter(has_vbg == 1) %>%
    select(all_of(outcome_vars), hyper_vbg) %>%
    tidyr::pivot_longer(all_of(outcome_vars), names_to = "outcome", values_to = "event") %>%
    group_by(hyper_vbg, outcome) %>%
    summarise(
      events = sum(event == 1, na.rm = TRUE),
      denom  = sum(!is.na(event)),
      rate   = events / denom,
    ...
    mutate(group = "VBG", hyper_group = as.character(hyper_vbg))
) %>%
  ungroup() %>%
  mutate(
    outcome      = recode(outcome, !!!outcome_labels),
    rate_fmt     = sprintf("%s/%s (%.1f%%)", events, denom, 100 * rate),
    hyper_group  = recode(hyper_group,
                          "Got ABG & Hypercapnia" = "Hypercapnia",
                          "Got ABG & No hypercapnia" = "No hypercapnia",
                          "Got VBG & Hypercapnia" = "Hypercapnia",
                          "Got VBG & No hypercapnia" = "No hypercapnia")
  ) %>%
  select(group, outcome, hyper_group, rate_fmt) %>%
  tidyr::pivot_wider(names_from = hyper_group, values_from = rate_fmt) %>%
  arrange(factor(group, levels = c("ABG", "VBG")),
          factor(outcome, levels = c("IMV", "NIV", "Death (60d)", "Hypercapnic RF")))

gt::gt(rate_panel) %>%
  gt::tab_header(title = "Table 2a. Crude outcomes by hypercapnia group") %>%
  gt::cols_label(
    group          = "Cohort",
    outcome        = "Outcome",
    Hypercapnia    = "Hypercapnia",
    `No hypercapnia` = "No hypercapnia"
  ) %>%
  gt::fmt_missing(columns = gt::everything(), missing_text = "—") %>%
  gt::tab_source_note("Counts and column percentages; denominator is non-missing records within each cohort.")
```

### 2.4 Generating Word Docs for Table 1 and Table 2 (separate files)
...
doc2 <- read_docx() %>%
  body_add_par("Table 2. Baseline summary by hypercapnia within ABG and VBG cohorts",
               style = "heading 1") %>%
...
vbg_rcs_forms <- list(
  "VBG RCS: IMV ~ CO2"      = imv_proc ~ rcs(vbg_co2, 4),
  "VBG RCS: NIV ~ CO2"      = niv_proc ~ rcs(vbg_co2, 4),
  "VBG RCS: Death60d ~ CO2" = death_60d ~ rcs(vbg_co2, 4),
  "VBG RCS: HCRF ~ CO2"     = hypercap_resp_failure ~ rcs(vbg_co2, 4)
)
...
ipw_abg_bin_forms <- list(
  "ABG IPW: IMV ~ has_abg"      = imv_proc ~ has_abg,
  "ABG IPW: NIV ~ has_abg"      = niv_proc ~ has_abg,
  "ABG IPW: Death60d ~ has_abg" = death_60d ~ has_abg,
  "ABG IPW: HCRF ~ has_abg"     = hypercap_resp_failure ~ has_abg
)
register_model_diagrams(ipw_abg_bin_forms)
...
v_bal <- suppressWarnings(bal.tab(
  w_vbg,
  un = TRUE,
  m.threshold = 0.1,
  binary = "std",
...
### Missing data / imputation summary (pre-imputation)
...
#| message: false
#| warning: false
# Key covariates + outcomes used in MI
miss_vars <- c(
  covars_gbm,
...
miss_vars <- intersect(miss_vars, names(subset_data))

miss_tbl <- subset_data |>
...
gt::gt(miss_tbl_disp) |>
  ...
  gt::tab_source_note(
    paste0(
      "n = ", nrow(subset_data),
      "; MI plan: m = 5 imputations (seed 20251206); methods: logreg (binary), polyreg (categorical), pmm (numeric)."
    )
  )
```
...
strata_vars <- c("has_abg", "has_vbg", "hypercap_on_abg", "hypercap_on_vbg",
                 "imv_proc", "death_60d", "encounter_type", "location")
strata_vars <- intersect(strata_vars, names(subset_data))

# Overall missingness %
miss_overall <- subset_data |>
...
miss_by_strata <- purrr::map_dfr(strata_vars, function(sv) {
  ...
  vars_here <- setdiff(intersect(miss_vars, names(subset_data)), sv)
  if (length(vars_here) == 0L) return(NULL)
  if (!sv %in% names(subset_data)) return(NULL)
...
utils::write.csv(miss_panel_full, miss_panel_file, row.names = FALSE)
...
kableExtra::kbl(
  ...
  longtable = FALSE,
  digits    = 1,
  col.names = c("Variable", "Stratum", "Level", "% missing in level", "% missing overall"),
  caption   = paste0(
...
vars_to_model <- miss_overall |>
  filter(pct_missing_overall > 0) |>
  pull(variable)
...
model_results <- purrr::map_dfr(vars_to_model, function(v) {
  ...
  if (dplyr::n_distinct(df$miss) < 2L) return(NULL)
  ...
  fit <- try(glm(fml, data = df, family = binomial()), silent = TRUE)
...
if (nrow(model_results) > 0) {
  ...
  utils::write.csv(model_results, model_results_file, row.names = FALSE)
  ...
  kableExtra::kbl(
    ...
    booktabs  = TRUE,
    longtable = FALSE,
    col.names = c("Target", "Predictor", "OR", "LCL", "UCL", "p"),
    caption   = paste0(
      "Predictors of missingness (logit OR; top 50 by p-value; full table saved to ",
      model_results_file, ")."
    )
  ) |>
    kableExtra::kable_styling(
...
if (!exists("imp") && file.exists(mi_mids_file)) {
  imp <- readRDS(mi_mids_file)
}
if (!exists("imp")) {
  message("MI object not available; skipping Monte Carlo error diagnostics.")
} else {
...
mc_err_vec <- if (exists("mice.mcerror")) {
  mice::mice.mcerror(mc_pool)$mcerror
} else {
  rep(NA_real_, nrow(mc_sum))
}
...
mc_tab <- mc_sum |>
  ...
  select(term, estimate, std.error, mc_error, mc_err_over_se, `2.5 %`, `97.5 %`)

gt::gt(mc_tab) |>
  gt::tab_header(title = "Monte Carlo error vs SE (should be much smaller)") |>
  gt::cols_label(
    term           = "Term",
    estimate       = "Estimate",
    std.error      = "SE",
...
imp_n <- imp$m
get_imp <- function(i, imp_obj = imp) mice::complete(imp_obj, action = i)
d1 <- get_imp(1)
...
```{r mi-diagnostics}
#| code-block-title: "MI diagnostics (trace + distributions)"
#| echo: false
#| message: false
#| warning: false
imp <- readRDS(mi_mids_file)
...
utils::write.csv(md_pat, md_pat_file, row.names = TRUE)
...
### 9.3 Observed vs imputed distributions (by strata)

```{r mi-imputation-diagnostics}
#| code-block-title: "Observed vs imputed distributions"
#| echo: false
#| message: false
#| warning: false

stopifnot(exists("imp"))
if (!exists("imp_n")) imp_n <- imp$m
if (!exists("get_imp")) get_imp <- function(i, imp_obj = imp) mice::complete(imp_obj, action = i)
...
if (length(vars_show)) {
  ...
  for (v in vars_show) {
    if (is.numeric(dat_long[[v]])) {
      p <- ggplot(dat_long, aes(x = .data[[v]], fill = .imp_label)) +
  ...
  if (length(strata_candidates)) {
    for (sv in strata_candidates) {
      for (v in vars_show) {
        ...
        p <- ggplot(dat_long, aes(x = .data[[sv]], y = .data[[v]], fill = .imp_label)) +
          geom_violin(alpha = 0.5, scale = "width", trim = TRUE) +
          geom_boxplot(width = 0.2, outlier.size = 0.6, position = position_dodge(width = 0.9)) +
...
```{r mi-propensity-abg}
#| cache: true
#| cache.extra: !expr digest::digest(list(if (exists("imp")) imp$m else NA, covars_gbm, gbm_params))
...
with_progress({
  ...
  # Avoid hitting future.globals.maxSize with large datasets
  options(future.globals.maxSize = +Inf)
  if (utils::object.size(imp$data) > 4e8) future::plan(sequential)
...
purrr::iwalk(outcomes_abg, function(fml, nm) {
  register_model_diagram(paste("MI IPW ABG:", nm, "~ has_abg"), fml)
})
...
progressr::with_progress({
  p <- progressr::progressor(steps = length(outcomes_abg) * imp_n)
  abg_results <- lapply(names(outcomes_abg), function(nm) {
    ...
    ests <- future.apply::future_lapply(
      ...
      FUN = function(i) {
        ...
        tryCatch(
          fit_and_extract(get_imp(i), W_abg_list[[i]]$weights, fml, "has_abg"),
...
purrr::iwalk(outcomes_vbg, function(fml, nm) {
  register_model_diagram(paste("MI IPW VBG:", nm, "~ has_vbg"), fml)
})
...
vbg_results <- NULL
progressr::with_progress({
  p <- progressr::progressor(steps = length(outcomes_vbg) * imp_n)
  vbg_results <- lapply(names(outcomes_vbg), function(nm) {
    ...
    ests <- future.apply::future_lapply(
      ...
      FUN = function(i) {
        ...
        tryCatch(
          fit_and_extract(get_imp(i), W_vbg_list[[i]]$weights, fml, "has_vbg"),
          error = function(e) NULL
        )
      },
      future.seed = TRUE
...
stopifnot(exists("imp"), exists("W_abg_list"), exists("W_vbg_list"))
imp_n <- imp$m
if (!exists("get_imp")) get_imp <- function(i, imp_obj = imp) mice::complete(imp_obj, action = i)
...
table3_df <- dplyr::bind_rows(
  ...
  dplyr::mutate(
    ...
    contrast = dplyr::recode(exposure,
                             Hypocapnia  = "Low vs normal",
                             Hypercapnia = "High vs normal",
                             .default    = exposure),
...
gt::gt(table3_wide) |>
  gt::tab_header(title = "Table 3. MI-pooled IPW associations between CO\u2082 category and outcomes") |>
  gt::cols_label(
    group           = "Cohort",
...
#| cache: true
#| cache.extra: !expr digest::digest(list(if (exists("imp")) imp$m else NA, gbm_params))
#| message: false
#| warning: false
#| fig-width: 10
#| fig-height: 8
...
mi_ipw_rcs_forms <- list(
  "MI IPW RCS: IMV ~ CO2 (ABG)"        = imv_proc ~ splines::ns(co2, 4),
  "MI IPW RCS: NIV ~ CO2 (ABG)"        = niv_proc ~ splines::ns(co2, 4),
  "MI IPW RCS: Death60d ~ CO2 (ABG)"   = death_60d ~ splines::ns(co2, 4),
  "MI IPW RCS: HCRF ~ CO2 (ABG)"       = hypercap_resp_failure ~ splines::ns(co2, 4),
  "MI IPW RCS: IMV ~ CO2 (VBG)"        = imv_proc ~ splines::ns(co2, 4),
  "MI IPW RCS: NIV ~ CO2 (VBG)"        = niv_proc ~ splines::ns(co2, 4),
  "MI IPW RCS: Death60d ~ CO2 (VBG)"   = death_60d ~ splines::ns(co2, 4),
  "MI IPW RCS: HCRF ~ CO2 (VBG)"       = hypercap_resp_failure ~ splines::ns(co2, 4)
)
...
rng_abg <- quantile(
  unlist(lapply(seq_len(imp_n), function(i) {
    d <- get_imp(i)
    as.numeric(d$paco2[d$has_abg == 1])
  })),
  c(.02, .98), na.rm = TRUE
)
rng_vbg <- quantile(
  unlist(lapply(seq_len(imp_n), function(i) {
    d <- get_imp(i)
    as.numeric(d$vbg_co2[d$has_vbg == 1])
  })),
  c(.02, .98), na.rm = TRUE
)
...
pool_rcs_curve <- function(group = c("ABG","VBG"), outcome,
  ...
  co2_all <- unlist(lapply(seq_len(imp_n), function(i) {
    d <- get_imp(i)
