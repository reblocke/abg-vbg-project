...
knitr::kable(seed_escrow, caption = "Seed escrow for MI and GBM runs")
...
fit_abg_one <- function(d, imp_index) {
  ...
  write_factor_levels_diag(d_ps, covars_ps, "abg", file_prefix = "mi_ps_factor_levels")
...
fit_vbg_one <- function(d, imp_index) {
  ...
  d_ps <- d[, c("has_abg", "has_vbg", covars_ps), drop = FALSE]
  d_ps <- droplevels_all(d_ps)
  write_factor_levels_diag(d_ps, covars_ps, "vbg", file_prefix = "mi_ps_factor_levels")
...
mi_ps_type_file <- results_path("mi_ps_covariate_types.csv")
d_ps_tmp <- get_imp(1)
...
mi_ps_types <- dplyr::bind_rows(
  ...
  dplyr::mutate(run_id = run_id, run_ts = run_ts)
write_csv_safely(mi_ps_types, mi_ps_type_file, row_names = FALSE)
...
for (i in seq_len(imp_n)) {
  ...
  set.seed(20251206 + i)
  W_abg_list[[i]] <- fit_abg_one(d, i)
  set.seed(30251206 + i)
  W_vbg_list[[i]] <- fit_vbg_one(d, i)
