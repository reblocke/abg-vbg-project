...
dat1 <- complete(imp, 1)
vars_show <- head(intersect(vars_incomplete, names(dat1)), 6)  # limit for plotting & ensure present in imp

# Density plots by imputation status
if (length(vars_show)) {
  bt <- function(x) paste0("`", x, "`")  # handle non-syntactic names safely
  print(densityplot(imp, as.formula(paste("~", paste(bt(vars_show), collapse = " + ")))))
  # Box/strip plots on first imputation, by exposure strata where present
  strata_candidates <- intersect(c("has_abg", "has_vbg", "imv_proc", "death_60d"), names(dat1))
      ...
      for (v in vars_show) {
        if (v %in% names(dat1)) {
          print(
            stripplot(
              imp,
              as.formula(paste(bt(v), "~", bt(sv))),
              subset = .imp == 1,
              main = paste("Imputed vs observed:", v, "by", sv)
            )
          )
          print(
            bwplot(
