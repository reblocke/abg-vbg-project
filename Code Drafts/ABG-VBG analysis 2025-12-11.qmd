...
# Float barrier: emit only when a float has just been produced
float_barrier_pending <- FALSE
mark_float_emitted <- function() {
  float_barrier_pending <<- TRUE
  invisible(TRUE)
}
float_barrier <- function() {
  if (!knitr::is_latex_output()) return(invisible(NULL))
  if (!isTRUE(float_barrier_pending)) return(invisible(NULL))
  float_barrier_pending <<- FALSE
...
```{r float-barrier-table1abg, include=FALSE}
float_barrier()
```
...
```{r float-barrier-table1-everyone, include=FALSE}
float_barrier()
```
...
```{r float-barrier-table2, include=FALSE}
float_barrier()
```
...
```{r float-barrier-spline-unweighted, include=FALSE}
float_barrier()
```
...
```{r float-barrier-missingness-strata, include=FALSE}
float_barrier()
```
...
```{r float-barrier-mi-or-plot, include=FALSE}
float_barrier()
```
...
```{r float-barrier-mi-spline, include=FALSE}
float_barrier()
```
