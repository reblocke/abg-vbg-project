...
# helper: summarize modeled sample size and events across imputations for one
# MI categorical outcome/cohort combination.
mi_counts_threelevel <- function(outcome, group, method) {
  imp_n <- imp$m
  counts <- lapply(seq_len(imp_n), function(i) {
    ...
    if (group == "ABG") {
      co2_var <- "paco2"
      treat_var <- "has_abg"
      w <- if (identical(method, "Weighted and adjusted")) get_mi_weight("ABG", i) else rep(1, nrow(d))
      low_cut <- ABG_CO2_LOW
      high_cut <- ABG_CO2_HIGH
    } else {
      co2_var <- "vbg_co2"
      treat_var <- "has_vbg"
      w <- if (identical(method, "Weighted and adjusted")) get_mi_weight("VBG", i) else rep(1, nrow(d))
      low_cut <- VBG_CO2_LOW
...
# helper: vectorize the MI count summary across all four categorical outcomes
# for one cohort and analysis track.
mi_counts <- function(group, method) {
  outcomes <- c("imv_proc", "niv_proc", "death_60d", "hypercap_resp_failure")
  purrr::map_dfr(outcomes, function(outcome) {
    cnt <- mi_counts_threelevel(outcome, group, method = method)
    dplyr::mutate(cnt,
                  method = method,
                  group = group,
                  outcome = label_outcome(outcome))
...
counts_tbl_cat3 <- dplyr::bind_rows(
  ...
  mi_counts("ABG", "Imputed"),
  mi_counts("VBG", "Imputed"),
  mi_counts("ABG", "Weighted and adjusted"),
  mi_counts("VBG", "Weighted and adjusted")
)
