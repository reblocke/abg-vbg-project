...
# helper: export the cohort-flow graph to both PNG and PDF while drawing the
# same built grob on each device to avoid device-specific layout drift.
...
flow_include <- if (knitr::is_latex_output() && file.exists(flow_pdf)) flow_pdf else flow_png
knitr::include_graphics(flow_include)
```
...
MI_BATCH_THRESHOLD <- if (IS_PILOT) {
...
# helper: allow a user or debugging session to preserve large MI scratch files
# across renders when they are explicitly investigating an MI issue.
keep_mi_transient_outputs <- function() {
  flag <- tolower(Sys.getenv("KEEP_MI_TRANSIENT", unset = ""))
  isTRUE(getOption("abgvbg.keep_mi_transient", FALSE)) ||
...
cleanup_mi_transient_outputs <- function() {
  if (keep_mi_transient_outputs()) return(invisible(FALSE))
...
print_plot_once <- function(p, plot_name, width = 8, height = 6, dpi = 300) {
  ...
  display_file <- if (knitr::is_latex_output()) file_pdf else file_png
  if (register_plot_file(plot_name, file_png)) {
    mark_float_emitted()
    knitr::include_graphics(display_file)
  } else {
    invisible(NULL)
  }
}
...
print_plot_force <- function(p, plot_name, width = 8, height = 6, dpi = 300) {
  ...
  mark_float_emitted()
  knitr::include_graphics(if (knitr::is_latex_output()) file_pdf else file_png)
}
...
# preferring PDF for LaTeX output and falling back to PNG if needed.
manifest_figure_path <- function(label) {
  ...
  if (!file.exists(path)) {
    fallback_ext <- if (identical(ext, "pdf")) "png" else ext
    path <- artifact_file_path(stub, fallback_ext, subdir)
  }
...
render_manuscript_figure <- function(label, width = "0.88\\linewidth") {
  ...
  if (knitr::is_latex_output()) {
    ...
    latex_path <- fs::path_rel(fig_path, start = qmd_dir)
    out <- c(
      "\\clearpage",
      "\\begin{center}",
