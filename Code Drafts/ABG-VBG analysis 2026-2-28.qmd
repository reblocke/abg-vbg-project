...
write_standalone_table_pdf <- function(df, caption, file_stub,
  ...
  table_blocks <- if (length(parts) == 1L) {
    build_kbl(parts[[1]], caption)
  } else {
    letters_part <- LETTERS[seq_along(parts)]
...
decorate_weighted_comparability <- function(built_graph, weighted_nodes,
                                            label_text,
                                            band_gp = grid::gpar(
                                              fill = grDevices::adjustcolor("grey92", alpha.f = 0.92),
                                              col = "grey82",
                                              lwd = 0.8
                                            ),
                                            text_gp = grid::gpar(col = "grey25", cex = 0.8),
                                            band_x = grid::unit(0.5, "npc"),
                                            band_y = grid::unit(0.13, "npc"),
                                            band_width = grid::unit(0.82, "npc"),
                                            band_height = grid::unit(0.18, "npc"),
                                            label_y = grid::unit(0.06, "npc")) {
  stopifnot(inherits(built_graph, "gTree"))
  stopifnot(length(weighted_nodes) == 2L)

  band_rect <- grid::roundrectGrob(
    x = band_x,
    y = band_y,
    width = band_width,
    height = band_height,
    r = grid::unit(2.5, "mm"),
    gp = band_gp,
    name = "weighted_comparability_band"
  )
...
#| message: false
#| warning: false
#| fig-cap: "Cohort flow diagram for the extracted 2022 encounter cohort, ambulatory exclusion, analytic ED/inpatient cohort, and ABG- and VBG-tested subcohorts, each weighted to the same analytic target cohort. Some encounters had both ABG and VBG testing (n = 55,049)."
#| fig-alt: "Flow diagram showing the extracted 2022 encounter cohort, ambulatory exclusions, the analytic emergency department or inpatient cohort, tested ABG and VBG subcohorts, and separate weighted pseudo-populations."
# Purpose: cohort flow figure.
...
analytic_base_box <- paste0(
  "Analytic ED or inpatient encounter cohort\n",
...
weighted_box <- paste0(
  "ABG-weighted pseudo-population\n",
  "standardized to target analytic cohort"
)
weighted_boxes <- c(
  weighted_box,
  paste0(
    "VBG-weighted pseudo-population\n",
    "standardized to target analytic cohort"
  )
)
...
cohort_flow_graph <- consort::add_box(txt = top_box, text_width = 34) |>
  ...
  consort::add_box(txt = analytic_base_box, text_width = 34) |>
  consort::add_split(txt = split_boxes, text_width = 24) |>
  consort::add_box(
    txt = weighted_boxes,
    text_width = 28,
    box_gp = grid::gpar(fill = "grey90", col = "grey45", lwd = 0.9)
  )
...
save_cohort_flow_outputs(
  ...
  decorate_fn = function(built_graph) {
    decorate_weighted_comparability(
      ...
      label_text = "Conditionally comparable after weighting on propensity covariates"
    )
  }
)
...
write_plot_alias <- function(plot_name, alias_name) {
  ...
  for (ext in c("png", "pdf")) {
    src <- results_path("figs", paste0(plot_name, ".", ext))
    dst <- results_path("figs", paste0(alias_name, ".", ext))
    if (file.exists(src)) {
  ...
  if (!length(copied)) {
    stop("No plot artifact found for alias source: ", plot_name)
  }
  invisible(copied)
}
...
ipw_abg_prob_panel <- (patchwork::wrap_plots(
  ...
  plot_annotation(
    title = paste0("Propensity‑weighted predicted probabilities by ABG CO2 (ref = ",
                   ABG_CO2_REF, "; conditional on X; 2–98% range)"),
...
ipw_vbg_prob_panel <- (patchwork::wrap_plots(
  ...
  plot_annotation(
    title = paste0("Propensity‑weighted predicted probabilities by VBG CO2 (ref = ",
                   VBG_CO2_REF, "; conditional on X)"),
...
ipw_overlay_prob_plot <- ggplot(ipw_curve_overlay_prob, aes(x = co2, y = p, color = group, fill = group)) +
  ...
  labs(
    title = expression(
      paste("IPSW-adjusted spline predicted probabilities: ABG vs VBG CO"[2], " (non-MI)")
    ),
    x = expression(CO[2]~"(mmHg)"),
    y = "Predicted probability",
    caption = paste0(
...
write_standalone_table_pdf(
  ...
  caption = "Gradient-boosted propensity-weighted associations of pCO2 category with clinical outcomes by ABG and VBG",
...
### Observed outcome rates by CO2 bin (IPSW, non-MI)

```{r co2-binned-rates-ipw-nonmi}
#| code-block-title: "Observed outcome rates by CO2 bin (IPSW, non-MI)"
#| fig-pos: "H"
...
#| fig-height: 5
#| fig-pos: "H"
#| fig-cap: "Tested-versus-untested propensity-score distributions under the gradient-boosted ABG and VBG test-ordering models."
#| fig-alt: "Two histograms showing the gradient-boosted propensity-score distributions for tested and untested encounters in the ABG and VBG ordering models."
...
p_ps_cond <- ggplot(df_ps_cond, aes(x = ps, fill = treat)) +
  ...
  labs(
    title = "Tested-versus-untested propensity-score distributions (GBM models)",
    x     = "Propensity score",
    y     = "Density",
    fill  = "Group"
  ) +
...
#| fig-height: 6
#| fig-pos: "H"
#| fig-cap: "Covariate balance after gradient-boosted propensity weighting for ABG and VBG test-ordering models. Points show absolute standardized mean differences before and after weighting; the dashed line marks |SMD| = 0.1."
#| fig-alt: "Two-panel love plot for ABG and VBG showing absolute standardized mean differences before and after gradient-boosted propensity weighting, with a dashed threshold at 0.1."
...
p_lov_nonmi <- loveplot_style(lov_nonmi_long, "Covariate balance after GBM weighting")
...
#| fig-height: 5
#| fig-pos: "H"
#| fig-cap: "SHAP-style contribution summaries for the gradient-boosted ABG and VBG test-ordering models. Bars show mean absolute SHAP values."
#| fig-alt: "Side-by-side bar charts listing the top mean absolute SHAP contributors in the ABG and VBG gradient-boosted test-ordering models."
...
p_shap <- plot_shap_top10_two_panel(
  shap_plot_df,
  "GBM test-ordering models: SHAP-style contribution summaries",
  "Mean |SHAP value|"
)
...
render_table_pdf(
  ...
  caption = "Categorical key results for the ABG cohort across crude, adjusted, imputed, weighted, and GBM-weighted analyses; probabilities are conditional on X_ref except in the crude track.",
...
render_table_pdf(
  ...
  caption = "Categorical key results for the VBG cohort across crude, adjusted, imputed, weighted, and GBM-weighted analyses; probabilities are conditional on X_ref except in the crude track.",
...
render_table_pdf(
  table_summary_abg,
  caption = "Adjusted odds ratios (low/high vs normal) across analysis tracks for ABG cohort; n/events reflect model sample size (median across imputations for MI).",
...
render_table_pdf(
  table_summary_vbg,
  caption = "Adjusted odds ratios (low/high vs normal) across analysis tracks for VBG cohort; n/events reflect model sample size (median across imputations for MI).",
...
key_results_cat3_caption_main <- paste0(
  "MI-pooled, IPSW-adjusted categorical odds ratios use the normal CO2 category as the reference. ",
  "Predicted probabilities are pooled on the link scale and are conditional on X_ref within each test-specific cohort."
)
...
key_results_cat3_main_plot <- assemble_cat3_key_results_figure(
  main_cat3_results,
  main_cat3_probs,
  figure_title = "MI-pooled IPSW-adjusted categorical associations: ABG vs VBG CO2 category",
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Canonical crude categorical key-results figure showing odds ratios and predicted probabilities for low, normal, and high pCO2 categories across four clinical outcomes for ABG and VBG."
#| fig-alt: "Eight-panel categorical key-results figure with crude ABG and VBG odds-ratio forest plots on the left and predicted-probability forest plots on the right across four outcomes."
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Canonical adjusted categorical key-results figure showing odds ratios and predicted probabilities for low, normal, and high pCO2 categories across four clinical outcomes for ABG and VBG."
#| fig-alt: "Eight-panel categorical key-results figure with adjusted ABG and VBG odds-ratio forest plots on the left and predicted-probability forest plots on the right across four outcomes."
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Canonical MI-pooled, non-weighted categorical key-results figure showing odds ratios and predicted probabilities for low, normal, and high pCO2 categories across four clinical outcomes for ABG and VBG."
#| fig-alt: "Eight-panel categorical key-results figure with MI-pooled non-weighted ABG and VBG odds-ratio forest plots on the left and predicted-probability forest plots on the right across four outcomes."
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Canonical MI-pooled, IPSW-adjusted categorical key-results figure showing odds ratios and predicted probabilities for low, normal, and high pCO2 categories across four clinical outcomes for ABG and VBG."
#| fig-alt: "Eight-panel categorical key-results figure with MI-pooled IPSW-adjusted ABG and VBG odds-ratio forest plots on the left and predicted-probability forest plots on the right across four outcomes."
# Purpose: canonical categorical key-results figure for main MI-weighted models.
print_plot_once(
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Canonical GBM-weighted categorical key-results figure showing odds ratios and predicted probabilities for low, normal, and high pCO2 categories across four clinical outcomes for ABG and VBG."
#| fig-alt: "Eight-panel categorical key-results figure with GBM-weighted ABG and VBG odds-ratio forest plots on the left and predicted-probability forest plots on the right across four outcomes."
...
## Observed outcome rates by CO2 bin (MI + IPSW)

```{r co2-binned-rates-mi-ipw}
#| code-block-title: "Observed outcome rates by CO2 bin (MI + IPSW)"
#| fig-pos: "H"
...
#| fig-height: 4.8
#| fig-pos: "H"
#| fig-cap: "MI-pooled, IPSW-adjusted odds ratios for low versus normal and high versus normal pCO2 categories across IMV, NIV, 60-day mortality, and hypercapnic respiratory failure, shown separately for ABG and VBG with 95% confidence intervals."
#| fig-alt: "Faceted odds-ratio plot comparing ABG and VBG low-versus-normal and high-versus-normal pCO2 categories across four outcomes, with confidence intervals."
...
#| fig-height: 4.9
#| fig-pos: "H"
#| fig-cap: "MI-pooled, IPSW-adjusted predicted probabilities across low, normal, and high pCO2 categories for IMV, NIV, 60-day mortality, and hypercapnic respiratory failure, shown separately for ABG and VBG."
#| fig-alt: "Faceted predicted-probability plot comparing ABG and VBG low, normal, and high pCO2 categories across four outcomes, with confidence intervals."
...
mi_cat_prob_plot <- ggplot(
  ...
  facet_wrap(~ outcome) +
  labs(
    title = "MI-pooled, IPW-adjusted predicted probabilities by PCO2 category (ABG vs VBG)",
    x = "PCO2 category",
    y = "Predicted probability",
...
#| fig-height: 5
#| fig-pos: "H"
#| fig-cap: "Imputation-averaged propensity-score distributions for tested and untested encounters in the MI logistic ABG and VBG test-ordering models."
#| fig-alt: "Two histograms showing imputation-averaged tested-versus-untested propensity-score distributions for the MI logistic ABG and VBG models."
...
#| fig-height: 6
#| fig-pos: "H"
#| fig-cap: "Covariate balance after MI logistic inverse-probability weighting for ABG and VBG test-ordering models. Points show median absolute standardized mean differences before and after weighting across imputations; the dashed line marks |SMD| = 0.1."
#| fig-alt: "Two-panel love plot for ABG and VBG showing median pre-weighting and post-weighting absolute standardized mean differences across imputations."
...
#| fig-height: 5
#| fig-pos: "H"
#| fig-cap: "SHAP-style contribution summaries for the MI logistic ABG and VBG test-ordering models. Bars show mean absolute contributions on the link scale."
#| fig-alt: "Side-by-side bar charts listing the top mean absolute contribution features in the MI logistic ABG and VBG ordering models."
# Purpose: render SHAP top-10 directly from CSV to ensure PDF display.
# Read precomputed top-10 SHAP values and re-plot deterministically.
...
#| fig-height: 6.5
#| fig-pos: "H"
#| fig-cap: "MI-pooled, IPSW-adjusted spline odds-ratio curves for ABG and VBG pCO2 across IMV, NIV, 60-day mortality, and hypercapnic respiratory failure. Shaded bands show 95% confidence intervals."
#| fig-alt: "Four-panel spline odds-ratio figure overlaying ABG and VBG curves with 95% confidence bands across four clinical outcomes."
...
#| fig-height: 6.5
#| fig-pos: "H"
#| fig-cap: "MI-pooled, IPSW-adjusted spline predicted-probability curves for ABG and VBG pCO2 across IMV, NIV, 60-day mortality, and hypercapnic respiratory failure. Shaded bands show 95% confidence intervals."
#| fig-alt: "Four-panel spline predicted-probability figure overlaying ABG and VBG curves with 95% confidence bands across four clinical outcomes."
...
mi_prob_overlay <- ggplot(curve_all_prob, aes(x = co2, y = p, color = group, fill = group)) +
  ...
  labs(
    title = expression(
      paste("MI-pooled, IPSW-adjusted spline predicted probabilities: ABG vs VBG CO"[2])
    ),
    x = expression(CO[2]~"(mmHg)"),
    y = "Predicted probability",
    caption = paste0(
...
key_results_adjusted_plot <- build_key_results_figure(
  ...
  figure_title = "Adjusted complete-case spline associations: ABG vs VBG CO2",
...
key_results_main_mi_ipw_plot <- build_key_results_figure(
  ...
  figure_title = "MI-pooled IPSW-adjusted spline associations: ABG vs VBG CO2",
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Canonical crude spline key-results figure showing odds-ratio and predicted-probability associations of pCO2 with IMV, NIV, 60-day mortality, and hypercapnic respiratory failure for ABG and VBG."
#| fig-alt: "Eight-panel key-results figure with odds-ratio panels on the left and predicted-probability panels on the right, comparing crude ABG and VBG spline associations across four outcomes."
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Unweighted covariate-adjusted spline associations of pCO2 with clinical outcomes by ABG and VBG. Odds-ratio curves are referenced to the midpoint of the normal range and predicted-probability curves are conditional on the reference covariate profile X_ref."
#| fig-alt: "Eight-panel key-results figure with adjusted ABG and VBG spline odds-ratio and predicted-probability curves across four outcomes."
# Purpose: canonical key-results figure for adjusted complete-case spline models.
print_plot_once(
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Canonical MI-pooled, non-weighted spline key-results figure showing odds-ratio and predicted-probability associations of pCO2 with four clinical outcomes for ABG and VBG."
#| fig-alt: "Eight-panel key-results figure with MI-pooled non-weighted ABG and VBG spline odds-ratio and predicted-probability curves across four outcomes."
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Canonical MI-pooled, IPSW-adjusted spline key-results figure showing odds-ratio and predicted-probability associations of pCO2 with four clinical outcomes for ABG and VBG."
#| fig-alt: "Eight-panel key-results figure with MI-pooled IPSW-adjusted ABG and VBG spline odds-ratio and predicted-probability curves across four outcomes."
# Purpose: canonical key-results figure for main MI-weighted spline models.
print_plot_once(
...
#| fig-height: 11
#| fig-pos: "H"
#| fig-cap: "Canonical GBM-weighted spline key-results figure showing odds-ratio and predicted-probability associations of pCO2 with four clinical outcomes for ABG and VBG."
#| fig-alt: "Eight-panel key-results figure with GBM-weighted ABG and VBG spline odds-ratio and predicted-probability curves across four outcomes."
...
#| code-block-title: "Supplement artifact manifest"
#| include: true
#| echo: true
#| message: false
# Purpose: freeze supplement numbering, paths, and targeted duplicate cleanup.
...
supplement_manifest_internal <- tibble::tribble(
  ~supp_label, ~title, ~analysis_family, ~artifact_file, ~chunk_name, ~notes,
  "Table S2", "Crude associations of pCO2 category with clinical outcomes by ABG and VBG", "crude", results_path("table_s2_crude_threelevel.pdf"), "supplement-table-s2-crude", "Standalone crude categorical odds-ratio table; paired CSV at Results/table_s2_crude_threelevel.csv.",
  "Figure S5", "Unweighted covariate-adjusted spline associations of pCO2 with clinical outcomes by ABG and VBG", "unweighted_adjusted", results_path("figs", "figure_s5_unweighted_adjusted_spline_abg_vbg.png"), "key-results-spline-adjusted-abg-vbg", "Alias of the canonical combined adjusted spline figure.",
  "Table S3", "Gradient-boosted propensity-weighted associations of pCO2 category with clinical outcomes by ABG and VBG", "gbm_sensitivity", results_path("table_s3_gbm_threelevel.pdf"), "supplement-table-s3-gbm", "Standalone GBM-weighted categorical odds-ratio table; paired CSV at Results/table_s3_gbm_threelevel.csv.",
  "Figure S6", "Covariate balance after gradient-boosted propensity weighting for ABG and VBG test-ordering models", "gbm_sensitivity", results_path("figs", "loveplot-ipw-gbm-abg-vbg.png"), "loveplot-ipw-gbm", "Raw vs weighted absolute standardized mean differences; dashed line marks |SMD| = 0.1.",
  "Figure S7", "Propensity-score overlap for gradient-boosted ABG and VBG test-ordering models", "gbm_sensitivity", results_path("figs", "propensity-histograms-conditional.png"), "propensity-histograms-conditional", "Tested-versus-untested propensity-score distributions under the GBM models.",
  "Figure S8", "SHAP-style contribution summaries for gradient-boosted ABG and VBG test-ordering models", "gbm_sensitivity", results_path("figs", "shap-top10-ipw-gbm-abg-vbg.png"), "shap-top10-ipw-gbm-display", "Mean absolute SHAP value summaries; not interpreted as independent predictors."
) |>
  dplyr::mutate(artifact_path = vapply(artifact_file, pretty_path, character(1L))) |>
  dplyr::select(supp_label, title, analysis_family, artifact_path, chunk_name, notes, artifact_file)
...
#| include: true
#| message: false
# Purpose: verify supplement artifacts are present and targeted spill duplicates are removed.
required_supplement_files <- c(
  results_path("table_s2_crude_threelevel.csv"),
  results_path("table_s2_crude_threelevel.pdf"),
  results_path("figs", "figure_s5_unweighted_adjusted_spline_abg_vbg.png"),
  results_path("table_s3_gbm_threelevel.csv"),
  results_path("table_s3_gbm_threelevel.pdf"),
  results_path("figs", "loveplot-ipw-gbm-abg-vbg.png"),
  results_path("figs", "propensity-histograms-conditional.png"),
  results_path("figs", "shap-top10-ipw-gbm-abg-vbg.png"),
  results_path("supplement_manifest.csv")
)

missing_supplement_files <- required_supplement_files[!file.exists(required_supplement_files)]
forbidden_duplicate_files <- c(
  results_path("figs", "propensity-histograms-conditional-1.png")
)
existing_forbidden_duplicates <- forbidden_duplicate_files[file.exists(forbidden_duplicate_files)]

if (length(missing_supplement_files) || length(existing_forbidden_duplicates)) {
  missing_pretty <- vapply(missing_supplement_files, pretty_path, character(1L))
  duplicate_pretty <- vapply(existing_forbidden_duplicates, pretty_path, character(1L))
  stop(
    "Supplement artifact check failed: missing files [",
    paste(missing_pretty, collapse = ", "),
    "]; forbidden duplicates [",
    paste(duplicate_pretty, collapse = ", "),
    "]."
  )
}
```
