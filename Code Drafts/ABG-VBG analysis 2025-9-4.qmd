...
data_dir_name <- '/Users/reblocke/Research/abg-vbg-project/data'
  #'/Users/blocke/Box Sync/Residency Personal Files/Scholarly Work/Locke Research Projects/abg-vbg-project/data' # 'data/' this is changed from just
...
```{r}
set.seed(123)
rows_to_keep <- round(nrow(stata_data) * 1.0)
...
ggplot(
  ...
  coord_flip() +
  labs(
    title  = "Odds Ratios of Outcomes by PCO₂ Category (ABG, VBG, Calc-ABG)",
    x      = "Outcome",
    y      = "Odds Ratio (log scale, 95% CI)",
    color  = "Blood-gas type",
    shape  = "PCO₂ category",
...
# ── 1. Create PCO2 categories for each cohort ────────────────────────────────
subset_data <- subset_data %>%
  mutate(
    pco2_cat_abg  = case_when(
      !is.na(paco2) & paco2 < 35 ~ "Below normal",
...
# ── 2. Function to fit weighted logistic regression and extract ORs ─────────
run_weighted_or <- function(data, outcome, cat_var, weight_var, group_name) {
  design <- svydesign(ids = ~1, weights = as.formula(paste0("~", weight_var)), data = data)
  ...
  tidy(fit, exponentiate = TRUE, conf.int = TRUE) %>%
    ...
    mutate(
      group    = group_name,
      outcome  = outcome,
      exposure = gsub(cat_var, "", term) %>% gsub("`", "", .)  # clean term name
    )
}

# ── 3. Loop over outcomes and cohorts ───────────────────────────────────────
outcomes <- c("imv_proc", "niv_proc", "death_60d", "hypercap_resp_failure")

combined_or_df <- bind_rows(
  lapply(outcomes, function(out) run_weighted_or(subset_data, out, "pco2_cat_abg",  "w_abg",  "ABG")),
  lapply(outcomes, function(out) run_weighted_or(subset_data, out, "pco2_cat_vbg",  "w_vbg",  "VBG")),
  lapply(outcomes, function(out) run_weighted_or(subset_data, out, "pco2_cat_calc", "w_vbg_calc", "Calculated ABG"))
)

# ── 4. Ensure proper factor levels for plotting ─────────────────────────────
combined_or_df$group <- factor(combined_or_df$group, levels = c("ABG", "VBG", "Calculated ABG"))
combined_or_df$exposure <- factor(combined_or_df$exposure, levels = c("Below normal", "Normal", "Above normal"))

# ── 5. Plot weighted odds ratios ────────────────────────────────────────────
ggplot(
  combined_or_df,
  aes(
    x      = outcome,
...
# Fit propensity model for ABG
w_abg <- weightit(
  has_abg ~ age_at_encounter + sex + factor(race_ethnicity) +
            curr_bmi + copd + asthma + osa + chf +
            acute_nmd + phtn + factor(location),
  data        = subset_data,
  method      = "gbm",
...
# Quick check
ls()            # should now include w_abg
head(w_abg$ps)  # first few propensity scores

# Build a combined dataframe of all PS + treatment flags
df_ps <- bind_rows(
  data.frame(
    ps        = w_abg$ps,
...
# Plot density histograms for each
ggplot(df_ps, aes(x = ps, fill = factor(treat))) +
...
```{r}
# --- 0) Harmonize hypercapnia definitions from raw CO2 -----------------------
# Align cut points with your plots: ABG/Calc-ABG >45 mmHg; VBG >50 mmHg
subset_data <- subset_data %>%
  mutate(
    calc_abg       = if_else(!is.na(vbg_co2) & !is.na(vbg_o2sat),
                             vbg_co2 - 0.22 * (93 - vbg_o2sat), NA_real_),
    hypercap_abg   = if_else(!is.na(paco2),     as.integer(paco2     > 45), NA_integer_),
    hypercap_vbg   = if_else(!is.na(vbg_co2),   as.integer(vbg_co2   > 50), NA_integer_),
    hypercap_calc  = if_else(!is.na(calc_abg),  as.integer(calc_abg  > 45), NA_integer_)
  )

# Optional cross-checks (catches reversed/bad encodings in existing columns)
if ("hypercap_on_abg" %in% names(subset_data)) {
  print(table(FromCO2=subset_data$hypercap_abg, Existing=subset_data$hypercap_on_abg, useNA="ifany"))
}
if ("hypercap_on_vbg" %in% names(subset_data)) {
  print(table(FromCO2=subset_data$hypercap_vbg, Existing=subset_data$hypercap_on_vbg, useNA="ifany"))
}
if ("hypercapnia_calc" %in% names(subset_data)) {
  print(table(FromCO2=subset_data$hypercap_calc, Existing=subset_data$hypercapnia_calc, useNA="ifany"))
}

# --- 1) Selection models (who gets measured), fit on the full dataset --------
library(WeightIt)
library(survey)
library(dplyr)
library(broom)

mk_ipsw <- function(formula, data, Svar, tail = 0.01) {
  # Fit selection (measurement) propensity via GBM
  wm <- weightit(
    formula,
    data        = data,
    method      = "gbm",
    estimand    = "ATE",         # gives 1/Pr(S=1|X) for the selected (S=1)
    missing.    = "ind",
    n.trees     = 3000,
    interaction.depth = 3,
    shrinkage   = 0.01,
  ...
  # Winsorize and re-stabilize **within the selected subset** only
  idx <- which(data[[Svar]] == 1L)
  cut <- quantile(w_all[idx], c(tail, 1 - tail), na.rm = TRUE)
  w_all[idx] <- pmin(pmax(w_all[idx], cut[1]), cut[2])
  w_all[idx] <- w_all[idx] / mean(w_all[idx], na.rm = TRUE)  # E[w | S=1] = 1

  list(w = w_all, wm = wm)
}

# Indicators for selection S (being measured)
subset_data <- subset_data %>%
  mutate(
    S_abg       = as.integer(!is.na(paco2)),                 # measured ABG CO2
    S_vbg       = as.integer(!is.na(vbg_co2)),               # measured VBG CO2
    S_vbg_calc  = as.integer(!is.na(vbg_co2) & !is.na(vbg_o2sat))  # Calc-ABG available
  )

# Covariate sets (match your earlier code)
cov_abg <- "age_at_encounter + sex + factor(race_ethnicity) + curr_bmi +
            copd + asthma + osa + chf + acute_nmd + phtn + factor(location) +
            temp_new + sbp + dbp + hr + spo2"

cov_vbg <- "age_at_encounter + sex + factor(race_ethnicity) + curr_bmi +
            copd + asthma + osa + chf + acute_nmd + phtn + factor(location)"

# Fit selection models on the full sample
ipsw_abg  <- mk_ipsw(as.formula(paste("S_abg ~",      cov_abg)), subset_data, "S_abg")
ipsw_vbg  <- mk_ipsw(as.formula(paste("S_vbg ~",      cov_vbg)), subset_data, "S_vbg")
ipsw_calc <- mk_ipsw(as.formula(paste("S_vbg_calc ~", cov_vbg)), subset_data, "S_vbg_calc")

# Attach weights as columns (all rows; we will subset for modeling)
subset_data$w_abg      <- ipsw_abg$w
subset_data$w_vbg      <- ipsw_vbg$w
subset_data$w_vbg_calc <- ipsw_calc$w

# --- 2) Outcome models (OR for hypercapnia) in the **measured** cohorts ------
tidy_ipw <- function(data, outcome, exposure, weight_var, group_label, outcome_label) {
  des <- svydesign(ids = ~1, weights = as.formula(paste0("~", weight_var)), data = data)
  fit <- svyglm(as.formula(paste0(outcome, " ~ ", exposure)), design = des, family = quasibinomial())
  tidy(fit, exponentiate = TRUE, conf.int = TRUE) %>%
    filter(term == exposure) %>%
    mutate(group = group_label, outcome = outcome_label)
}

abg_df  <- subset_data %>% filter(S_abg      == 1L) %>% mutate(hypercap = hypercap_abg)
vbg_df  <- subset_data %>% filter(S_vbg      == 1L) %>% mutate(hypercap = hypercap_vbg)
calc_df <- subset_data %>% filter(S_vbg_calc == 1L) %>% mutate(hypercap = hypercap_calc)

ipw_estimates <- bind_rows(
  # ABG (weights = w_abg)
  tidy_ipw(abg_df,  "imv_proc",              "hypercap", "w_abg",      "ABG",           "Intubation"),
  tidy_ipw(abg_df,  "niv_proc",              "hypercap", "w_abg",      "ABG",           "NIV"),
  tidy_ipw(abg_df,  "death_60d",             "hypercap", "w_abg",      "ABG",           "Death"),
  tidy_ipw(abg_df,  "hypercap_resp_failure", "hypercap", "w_abg",      "ABG",           "ICD Code"),

  # VBG (weights = w_vbg)
  tidy_ipw(vbg_df,  "imv_proc",              "hypercap", "w_vbg",      "VBG",           "Intubation"),
  tidy_ipw(vbg_df,  "niv_proc",              "hypercap", "w_vbg",      "VBG",           "NIV"),
  tidy_ipw(vbg_df,  "death_60d",             "hypercap", "w_vbg",      "VBG",           "Death"),
  tidy_ipw(vbg_df,  "hypercap_resp_failure", "hypercap", "w_vbg",      "VBG",           "ICD Code"),

  # Calculated ABG (weights = w_vbg_calc)
  tidy_ipw(calc_df, "imv_proc",              "hypercap", "w_vbg_calc", "Calculated ABG","Intubation"),
  tidy_ipw(calc_df, "niv_proc",              "hypercap", "w_vbg_calc", "Calculated ABG","NIV"),
  tidy_ipw(calc_df, "death_60d",             "hypercap", "w_vbg_calc", "Calculated ABG","Death"),
  tidy_ipw(calc_df, "hypercap_resp_failure", "hypercap", "w_vbg_calc", "Calculated ABG","ICD Code")
)
...
```{r}
# 1) Check that binary exposure matches the threshold used in plots
with(abg_df, summary(tapply(paco2, hypercap, mean, na.rm = TRUE)))    # means should be higher when hypercap==1
with(vbg_df, summary(tapply(vbg_co2, hypercap, mean, na.rm = TRUE)))
with(calc_df, summary(tapply(calc_abg, hypercap, mean, na.rm = TRUE)))

# 2) PS overlap for selection (optional)
library(ggplot2)
ggplot(data.frame(ps = ipsw_abg$wm$ps, S = subset_data$S_abg),
       aes(ps, fill = factor(S))) + geom_histogram(position = "identity", bins = 30, alpha = 0.4)

# 3) Balance of covariates for selection (optional)
library(cobalt)
love.plot(bal.tab(ipsw_abg$wm,  un = TRUE, m.threshold = 0.05),
          abs=TRUE, var.order="unadjusted", sample.names=c("Raw","IPSW"))
```
