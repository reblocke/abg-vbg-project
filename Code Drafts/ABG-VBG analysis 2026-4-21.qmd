...
# Canonical PDF path: export all figure/table artifacts and include analysis
# previews plus manuscript-facing assets in the rendered notebook.
VALIDATION_INLINE_ANALYSIS_PREVIEWS <- TRUE
...
tbl1_combined_csv <- tbl1_abg_csv |>
  dplyr::select(
...
table_2_weighted_categorical_outcomes <- table_summary_key_results_cat3 |>
...
key_results_cat3_main_plot <- assemble_cat3_key_results_figure(
  ...
  figure_caption = key_results_cat3_caption_main
)
...
key_results_adjusted_plot <- build_key_results_figure(
  ...
  figure_caption = key_results_caption_adjusted
)
...
key_results_main_mi_ipw_plot <- build_key_results_figure(
  ...
  figure_caption = key_results_caption_main
)
...
manuscript_asset_manifest_internal <- tibble::tribble(
  ...
  "Supplementary Table 1", "Supplementary Table 1", "Inclusion criteria into the enriched sample", "table", "supplement", "", "external/manual", "external/manual", TRUE, "", FALSE, "", "info", "Defined outside the notebook; intentionally not generated here.", "external", "", "", "", "", "",
...
has_external_s1 <- any(
  manuscript_asset_manifest_internal$manuscript_label_safe == "Supplementary Table 1" &
    manuscript_asset_manifest_internal$status == "external/manual"
)
...
if (
  ...
  stop(
    ...
    "]; Supplementary Table 1 present [", has_external_s1,
