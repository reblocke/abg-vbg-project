...
predict_cat3_prob_from_compact <- function(fit_compact, grid_df, outcome_var,
  ...
  if (is.null(bn) || any(!nzchar(bn))) {
    if (length(beta) == ncol(mm)) {
      ...
      stop("predict_cat3_prob_from_compact: coefficient names missing and dimensions do not match.")
    }
  }
...
table_2_weighted_categorical_outcomes <- dplyr::bind_rows(
  ...
  table_2_weighted_probability_panel |>
    ...
    dplyr::select(.data$panel, .data$group, .data$outcome, .data$n_model, .data$events, .data$measure, .data$estimate)
)
...
write_standalone_panel_table_pdf(
  ...
  caption = "MI-pooled, MI-logistic IPSW-weighted 3-level categorical results for ABG and VBG",
...
render_table_pdf_maybe(
  ...
  caption = "Categorical key results for the ABG cohort across crude, unweighted adjusted, imputed non-weighted, MI-logistic IPSW-weighted, and GBM IPSW-weighted analyses; probabilities are conditional on X_ref except in the crude track.",
...
render_table_pdf_maybe(
  ...
  caption = "Categorical key results for the VBG cohort across crude, unweighted adjusted, imputed non-weighted, MI-logistic IPSW-weighted, and GBM IPSW-weighted analyses; probabilities are conditional on X_ref except in the crude track.",
...
mi_cat_prob_plot <- ggplot(
  ...
  labs(
    ...
    x = "PCO2 category",
    y = "Predicted probability",
    caption = "Conditional on reference profiles X_ref within each test-specific cohort."
  ) +
  theme_pub(base_size = 10)
...
probability scale, residual capture/missingness, support, setting, subgroup, or
timing structure.
...
spline_current_summary_one <- function(group, outcome_var) {
  ...
  prob_df <- if (identical(group, "ABG")) abg_prob_curves else vbg_prob_curves
  or_df <- or_df |>
...
if (is.na(site_field)) {
  record_discordance_status(
    ...
    "Results/discordance_setting_sensitivity.csv",
    optional = TRUE
  )
}
...
discordance_interpretation_summary <- tibble::tibble(
  hypothesis = c(
    ...
    "Dual-tested measurement structure",
