...
```{r}
set.seed(123)
rows_to_keep <- round(nrow(stata_data) * 0.1)
...
mkpred <- function(fit, data_ref) {
  ...
  q <- quantile(data_ref$paco2, probs = c(0.02, 0.99), na.rm = TRUE)
  newd <- data.frame(
...
mkpred <- function(fit, data_ref, xvar, group_label, n = 200) {
  ...
  rng <- quantile(data_ref[[xvar]], probs = c(0.02, 0.99), na.rm = TRUE)
...
Three Groups with Weights - Deleted VBG calc
