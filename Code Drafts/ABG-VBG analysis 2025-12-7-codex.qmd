...
subset_data <- subset_data %>%
  mutate(
    ...
    pco2_cat_calc = case_when(
      !is.na(calc_abg) & calc_abg < 35 ~ "Hypocapnia",
      !is.na(calc_abg) & calc_abg > 45 ~ "Hypercapnia",
      !is.na(calc_abg)                 ~ "Eucapnia"
    )
  ) %>%
  mutate(
...
combined_or_df <- results %>%
  mutate(
    exposure = recode(term,
                      ...
                      "pco2_cat_calcHypocapnia"  = "Hypocapnia",
                      "pco2_cat_calcHypercapnia" = "Hypercapnia"),
...
shap_y_calc <- "Contribution to log-odds of receiving a calculated ABG"
```
...
cell_plot <- function(v_row, v_col, i, j, n) {
  ...
  if (identical(v_row, v_col)) {
    ...
    ggplot(df, aes(x = x, y = shap)) +
      ...
      labs(title = v_row, x = v_row, y = shap_y_abg) +
...
cell_plot_vbg <- function(v_row, v_col, i, j, n) {
  ...
  if (identical(v_row, v_col)) {
    ...
    ggplot(df, aes(x = x, y = shap)) +
      ...
      labs(title = v_row, x = v_row, y = shap_y_vbg) +
    ...
    shapviz::sv_dependence(sv_vbg, v = v_row, color_var = v_col, size = 0.4) +
      scale_y_continuous(limits = y_rng_vbg) +
      labs(title = paste0(v_row, " | color: ", v_col), x = v_row, y = shap_y_vbg) +
      theme_axes_compact(show_y, show_x, base = 8) +
...
cell_plot_calc <- function(v_row, v_col, i, j, n) {
  ...
  if (identical(v_row, v_col)) {
    ...
    ggplot(df, aes(x = x, y = shap)) +
      ...
      labs(title = v_row, x = v_row, y = shap_y_calc) +
      theme_axes_compact(show_y, show_x, base = 8) +
      theme(legend.position = "none")
  } else {
    ...
    shapviz::sv_dependence(sv_calc, v = v_row, color_var = v_col, size = 0.4) +
      scale_y_continuous(limits = y_rng_calc) +
      labs(title = paste0(v_row, " | color: ", v_col), x = v_row, y = shap_y_calc) +
      theme_axes_compact(show_y, show_x, base = 8) +
      guides(colour = guide_colorbar(barheight = unit(24, "pt"), barwidth = unit(3, "pt"),
                                     title.position = "top", title.hjust = 0.5, label.position = "right")) +
      theme(legend.position = if (keep_legend) "right" else "none")
...
subset_data <- subset_data %>%
  mutate(
    ...
    pco2_cat_calc = case_when(
      !is.na(calc_abg) & calc_abg < 35 ~ "Hypocapnia",
      !is.na(calc_abg) & calc_abg >= 35 & calc_abg <= 45 ~ "Eucapnia",
      !is.na(calc_abg) & calc_abg > 45 ~ "Hypercapnia",
      TRUE ~ NA_character_
    )
  )
...
# --- Robust CO2→3-category helper ---------------------------------------------
make_co2_cat <- function(x, normal = c(35, 45)) {
  x <- suppressWarnings(as.numeric(x))
  # sanitize "normal"
  n <- sort(unique(as.numeric(normal)))
  finitex <- x[is.finite(x)]
  if (length(n) < 2 || any(!is.finite(n)) || (n[2] - n[1]) <= 0) {
    qs <- as.numeric(stats::quantile(finitex, probs = c(0.35, 0.65), na.rm = TRUE))
    n  <- sort(unique(qs))
    if (length(n) < 2 || any(!is.finite(n)) || (n[2] - n[1]) <= 0) {
      med <- stats::median(finitex, na.rm = TRUE)
      n <- c(med - 1, med + 1)
    }
  }
  # enforce strictly increasing breaks
  if (n[2] <= n[1]) n[2] <- n[1] + 1e-8
  cut(x,
      breaks = c(-Inf, n, Inf),
