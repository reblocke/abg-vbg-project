...
required_pkgs <- c(
  ...
  "modelsummary", "officer", "patchwork", "rms", "survey", "tibble"
)
...
subset_data_clean <- subset_data %>%
  select(paco2, vbg_co2, imv_proc, niv_proc, died, hypercap_resp_failure)
...
fit_death <- lrm(died ~ rcs(paco2, 4), data = subset_data_clean)
...
subset_data_vbg <- subset_data %>%
  select(vbg_co2, imv_proc, niv_proc, died, hypercap_resp_failure) %>%
...
fit_death_vbg <- lrm(died ~ rcs(vbg_co2, 4), data = subset_data_vbg)
...
subset_data_abg <- subset_data %>%
  select(calc_abg, imv_proc, niv_proc, died, hypercap_resp_failure) %>%
...
fit_death_abg  <- lrm(died ~ rcs(calc_abg, 4), data = subset_data_abg)
...
fit_death <- svyglm(died       ~ has_abg, design = design, family = quasibinomial())
...
#   )
# 
# bal.tab(w_abg, un = TRUE, m.threshold = 0.1)   # check balance
#  
...
#   filter(!is.na(paco2) & complete.cases(.))
#  
# dd <- datadist(subset_data_abg); options(datadist = "dd")
#  
# fit_imv   <- lrm(imv_proc             ~ rcs(paco2, 4),
#                      data = subset_data_abg, weights = ipw)
#  
# fit_niv   <- lrm(niv_proc             ~ rcs(paco2, 4),
#                      data = subset_data_abg, weights = ipw)
#  
# fit_death <- lrm(died                 ~ rcs(paco2, 4),
#                      data = subset_data_abg, weights = ipw)
#  
# fit_hcrf  <- lrm(hypercap_resp_failure ~ rcs(paco2, 4),
#                      data = subset_data_abg, weights = ipw)
#  
# pred_imv_abg   <- as.data.frame(Predict(fit_imv,   paco2, fun = plogis))
# pred_niv_abg   <- as.data.frame(Predict(fit_niv,   paco2, fun = plogis))
# pred_death_abg <- as.data.frame(Predict(fit_death, paco2, fun = plogis))
# pred_hcrf_abg  <- as.data.frame(Predict(fit_hcrf,  paco2, fun = plogis))
#  
...
BL: A new version that mirrors the same changes as above.
