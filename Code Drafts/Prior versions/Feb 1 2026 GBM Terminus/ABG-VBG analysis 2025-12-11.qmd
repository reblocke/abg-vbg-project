...
params:
  ...
  pilot_frac: 0.12 # used only when run_mode == "pilot"
...
# helper: render tables for PDF (preview + CSV)
render_table_pdf <- function(df, caption, file_stub,
  ...
  df_print <- df
  if (is.null(preview_cols) && (wide || ncol(df_print) > 8)) {
    preview_cols <- names(df_print)[seq_len(min(8, ncol(df_print)))]
  }
  if (!is.null(preview_cols)) {
    df_print <- df_print[, preview_cols, drop = FALSE]
  }
  note <- NULL
  if (nrow(df_print) > preview_rows || wide) {
    df_print <- utils::head(df_print, preview_rows)
    note <- paste0("Preview (top ", preview_rows, " rows). Full table saved to ",
                   pretty_path(out_csv), ".")
  }
  ...
  kbl <- knitr::kable(
    ...
    booktabs  = TRUE,
    longtable = FALSE,
    caption   = strip_manual_table_number(caption),
    digits    = digits
  )
  font_size_use <- if (wide || ncol(df_print) > 8) 6 else 7
  kbl <- kableExtra::kable_styling(
    kbl,
    latex_options = c("hold_position", "scale_down"),
    font_size     = font_size_use,
    full_width    = TRUE,
    position      = "center"
  )
...
required_pkgs <- c(
  ...
  "fastshap", "shapviz", "ggrepel", "digest"
)
...
summarize_warnings_log <- function(path = results_path("warnings_log.csv")) {
  ...
  warn_df |>
    ...
    dplyr::slice_head(n = min(10, nrow(warn_df)))
}
...
write_factor_levels_diag <- function(df, vars, context_id) {
  if (context_id %in% factor_diag_written) {
  ...
  out_path <- results_path(paste0("gbm_preflight_factor_levels_", context_id, ".csv"))
  write_csv_safely(diag, out_path, row_names = FALSE)
  factor_diag_written <<- c(factor_diag_written, context_id)
  invisible(TRUE)
}
...
gbm_preflight <- function(df, covars, context_id, sample_n = 50000L) {
  ...
  write_factor_levels_diag(df, covars, context_id)
  covars <- intersect(covars, names(df))
  if (!length(covars)) return(invisible(FALSE))
  n_full <- nrow(df)
  n_samp <- min(sample_n, n_full)
...
factor_levels_top <- factor_diag |>
  dplyr::filter(!is.na(nlevels)) |>
  dplyr::arrange(dplyr::desc(nlevels)) |>
  dplyr::slice_head(n = 10)
factor_missing_top <- factor_diag |>
  dplyr::arrange(dplyr::desc(n_missing)) |>
  dplyr::slice_head(n = 10)
knitr::kable(factor_levels_top, caption = "Top 10 factor variables by number of levels")
knitr::kable(factor_missing_top, caption = "Top 10 variables by missingness (count)")
...
# Combine overall + strata (save full table; display a compact PDF-friendly view)
top_miss <- miss_overall |>
  arrange(desc(pct_missing_overall)) |>
  slice_head(n = 10) |>
  pull(variable)

strata_show <- c("has_abg", "has_vbg", "imv_proc", "death_60d")
...
miss_panel_disp <- miss_panel_full |>
  filter(variable %in% top_miss, stratum %in% strata_show)

miss_caption <- paste0(
  "Missingness by key strata (pre-imputation; top 10 variables; full table saved to ",
  pretty_path(miss_panel_file), ")."
)
render_table_pdf(
  miss_panel_disp,
  caption = miss_caption,
  file_stub = "missingness_by_strata_preview",
  preview_rows = 20,
  digits = 1
)
float_barrier()
...
if (nrow(model_results) > 0) {
  ...
  render_table_pdf(
    model_results_disp,
    caption = paste0(
      "Predictors of missingness (logit OR; top 20 by p-value; full table saved to ",
      pretty_path(model_results_file), ")."
    ),
    file_stub = "missingness_drivers_top20",
    preview_rows = 20,
    digits = 2
  )
} else {
...
if (nrow(mi_obs_imp_summary)) {
  ...
  knitr::kable(head(mi_obs_imp_summary, 20),
               caption = "Observed vs imputed summaries (first 20 rows)")
} else {
  ...
  if (length(strata_candidates)) {
    ...
    for (sv in strata_candidates) {
      for (v in vars_show) {
        ...
        if (!printed_counts) {
          print(table(strata = df_plot$.strata, source = df_plot$.imp_label))
          printed_counts <- TRUE
        }
...
knitr::kable(mc_prog, caption = "MC error progress by m (early-stop diagnostic)")
```
...
for (i in seq_len(imp$m)) {
  di <- get_imp(i)
  assert_no_na_covars(di, covars_gbm, context = paste0("imputation ", i))
}
```
...
fit_abg_one <- function(d) {
  ...
  d_gbm <- d[, c("has_abg", "has_vbg", covars_gbm), drop = FALSE]
  d_gbm <- droplevels_all(d_gbm)
  assert_no_na_covars(d_gbm, covars_gbm, context = "ABG WeightIt (MI)")
  gbm_preflight(d_gbm, covars_gbm, "mi_abg")
  append_mem_snapshot("mi_gbm", "mi_abg", "pre")
  w <- weightit(
    formula_abg,
    data   = d_gbm,
    method = "gbm",
    ...
    n.cores           = get_gbm_cores()
  )
  append_mem_snapshot("mi_gbm", "mi_abg", "post")
  ...
  assert_finite_weights(ipow$weights[d_gbm$has_abg == 1], "w_abg")
...
fit_vbg_one <- function(d) {
  ...
  d_gbm <- d[, c("has_abg", "has_vbg", covars_gbm), drop = FALSE]
  d_gbm <- droplevels_all(d_gbm)
  assert_no_na_covars(d_gbm, covars_gbm, context = "VBG WeightIt (MI)")
  gbm_preflight(d_gbm, covars_gbm, "mi_vbg")
  append_mem_snapshot("mi_gbm", "mi_vbg", "pre")
  w <- weightit(
    formula_vbg,
    data   = d_gbm,
    method = "gbm",
  ...
  append_mem_snapshot("mi_gbm", "mi_vbg", "post")
  ipow <- compute_ipow_weights(
    w,
    treat = d_gbm$has_vbg,
    ps_floor_quantile = ps_trunc_quantile,
  ...
  assert_finite_weights(ipow$weights[d_gbm$has_vbg == 1], "w_vbg")
  w$weights <- ipow$weights
  w$ipow_info <- list(
    ps_floor = ipow$ps_floor,
    cap      = ipow$cap,
...
covars_use <- intersect(covars_gbm, names(subset_data))
covars_use_abg <- covars_use
covars_use_vbg <- covars_use

W_abg_list <- vector("list", imp_n)
W_vbg_list <- vector("list", imp_n)
...
for (i in seq_len(imp_n)) {
  ...
  set.seed(20251206 + i)
  W_abg_list[[i]] <- fit_abg_one(d)
  set.seed(30251206 + i)
  W_vbg_list[[i]] <- fit_vbg_one(d)

  bal_rows_abg[[i]] <- target_balance_table(
    d, "has_abg", W_abg_list[[i]]$weights, covars_use_abg, levels_ref
  ) |>
    dplyr::mutate(group = "ABG", imp = i)
  bal_rows_vbg[[i]] <- target_balance_table(
    d, "has_vbg", W_vbg_list[[i]]$weights, covars_use_vbg, levels_ref
  ) |>
    dplyr::mutate(group = "VBG", imp = i)

  for (outcome_var in cat3_outcomes) {
    fit_abg_cat <- fit_cat3_imp(
      d, W_abg_list[[i]]$weights, outcome_var, "paco2", "has_abg",
    ...
    fit_vbg_cat <- fit_cat3_imp(
      d, W_vbg_list[[i]]$weights, outcome_var, "vbg_co2", "has_vbg",
...
knitr::kable(round(mi_weight_diag_abg_sum, 3), caption = "ABG weight diagnostics (MI; median across imputations)")
...
mi_weight_diag_vbg_sum <- mi_weight_diag_vbg_df |>
  dplyr::summarise(dplyr::across(where(is.numeric), ~ stats::median(.x, na.rm = TRUE)))
knitr::kable(round(mi_weight_diag_vbg_sum, 3), caption = "VBG weight diagnostics (MI; median across imputations)")
...
```{r shap-backend-helpers}
#| code-block-title: "SHAP diagnostics (ABG/VBG)"
...
write_csv_safely(shap_stability, shap_stability_file, row_names = FALSE)
...
runtime_log <- dplyr::bind_rows(
  ...
  data.frame(
    ...
    start_time = as.character(shap_t0),
    end_time = as.character(shap_t1),
...
stopifnot(exists("imp"))
stopifnot(exists("W_abg_list"), exists("W_vbg_list"))
imp_n <- imp$m
...
gt::gt(table3_wide) |>
  ...
  gt::tab_source_note(
    paste0(
      "Weighted survey GLMs adjusted for baseline covariates; weights = MI-specific GBM IPW; m = ", M_IMP,
...
extra_miss_vars <- intersect(c("bnp", "spo2"), names(subset_data_raw))
miss_vars <- intersect(
  c(covars_gbm, extra_miss_vars, "paco2", "vbg_co2", "vbg_o2sat",
    "imv_proc", "niv_proc", "death_60d", "hypercap_resp_failure"),
...
if (nrow(log_events_summary) && !("empty" %in% names(log_events_summary) && all(log_events_summary$empty))) {
  top_events <- log_events_summary |>
    dplyr::slice_head(n = min(20, nrow(log_events_summary)))
  knitr::kable(top_events, caption = "Top logged events (MI)")
...
if (nrow(top_flagged)) {
  top_flagged <- dplyr::slice_head(top_flagged, n = min(10, nrow(top_flagged)))
}
if (nrow(top_flagged)) {
  knitr::kable(
    top_flagged |>
    ...
    caption = "MICE chain diagnostics (top flagged drift)"
  )
} else {
...
if (nrow(top_tail_na)) {
  top_tail_na <- dplyr::slice_head(top_tail_na, n = min(10, nrow(top_tail_na)))
}
if (nrow(top_tail_na)) {
  knitr::kable(top_tail_na, caption = "MICE chain diagnostics (highest tail NA fraction)")
}
...
stab_rows <- lapply(m_vals, function(mv) {
  ...
  fits_abg <- lapply(seq_len(mv), function(i) {
    ...
    tryCatch(
      fit_spline_imp(
        d, W_abg_list[[i]]$weights, "imv_proc", "paco2", "has_abg",
        adj_vars = adj_core,
  ...
  fits_vbg <- lapply(seq_len(mv), function(i) {
    ...
    tryCatch(
      fit_spline_imp(
        d, W_vbg_list[[i]]$weights, "imv_proc", "vbg_co2", "has_vbg",
        adj_vars = adj_core,
...
knitr::kable(ps_summary, caption = "Propensity score overlap (tested cohort)")
...
knitr::kable(bal_worst, caption = "Target balance (top 10 by max |SMD|)")
...
knitr::kable(worst_rows, caption = "Worst target SMD rows across imputations (top 10)")
...
if (nrow(bal_imp_summary)) {
  ...
  knitr::kable(dist_tbl, caption = "Distribution of max |Target SMD| across imputations")
}
if (nrow(worst_terms_by_imp)) {
  worst_freq <- worst_terms_by_imp |>
    ...
    dplyr::slice_head(n = 10) |>
    dplyr::ungroup()
  knitr::kable(worst_freq, caption = "Most frequent worst-balance terms (top 10 per group)")
}
...
audit_lines <- c(
  ...
  "## A3. GBM propensity weighting",
  "- WeightIt with method = \"gbm\" and balance-based stopping (stop.method = \"smd.max\"); no AUC-based tuning (`propensity-config`, `ipw-abg-weighting`, `ipw-vbg-workflow`, MI blocks).",
  "- SHAP summaries are computed on the log-odds (link) scale for the GBM propensity models; stability is checked with two seeds (`shap-abg-vbg-main`).",
...
if (nrow(warn_df)) {
  msg_counts <- warn_df |>
    ...
    dplyr::slice_head(n = min(10, nrow(warn_df)))
  knitr::kable(msg_counts, caption = "Top warning messages by stage/component")
  ...
  if (nrow(nonconv)) {
    nonconv_ctx <- nonconv |>
      ...
      dplyr::slice_head(n = min(10, nrow(nonconv)))
    knitr::kable(nonconv_ctx, caption = "Nonconvergence contexts (top 10)")
  }
} else {
...
sep <- mi_outcome_diag |>
  ...
  if (nrow(nonconv)) {
    ...
    knitr::kable(nonconv_counts, caption = "Nonconverged fits by variant/group/outcome")

    worst_imps <- nonconv |>
      dplyr::count(imputation, sort = TRUE) |>
      dplyr::slice_head(n = min(10, nrow(nonconv)))
    knitr::kable(worst_imps, caption = "Imputations with most nonconverged fits (top 10)")
  } else {
  ...
  if (nrow(sep)) {
    sep_counts <- sep |>
      dplyr::count(analysis_variant, group, outcome, model_type, sort = TRUE)
    knitr::kable(sep_counts, caption = "Separation flags by variant/group/outcome")
  } else {
  ...
  worst_phat <- mi_outcome_diag |>
    ...
    dplyr::slice_head(n = min(10, nrow(mi_outcome_diag)))
  if (nrow(worst_phat)) {
    knitr::kable(worst_phat, caption = "Worst fitted-probability extremes (top 10)")
  }
...
if (nrow(mi_warn_log)) {
  warn_top <- mi_warn_log |>
    dplyr::count(stage, component, analysis_variant, model_type, message, sort = TRUE) |>
    dplyr::slice_head(n = min(10, nrow(mi_warn_log)))
  knitr::kable(warn_top, caption = "Top warnings by stage/component")
}

# Top MICE loggedEvents drivers
stopifnot(exists("log_events_summary"))
if (nrow(log_events_summary)) {
  le_top <- log_events_summary |>
    dplyr::slice_head(n = min(10, nrow(log_events_summary)))
  knitr::kable(le_top, caption = "Top MICE loggedEvents drivers")
}
...
if (MI_DEBUG_REPLAY) {
  ...
  if (nrow(bad)) {
    ...
    if (row$group == "ABG") {
      ...
      w_dbg <- W_abg_list[[row$imputation]]$weights[d_dbg$has_abg == 1 & is.finite(d_dbg$paco2)]
      ...
      w_dbg <- W_vbg_list[[row$imputation]]$weights[d_dbg$has_vbg == 1 & is.finite(d_dbg$vbg_co2)]
      if (row$model_type == "spline") {
...
runtime_summary <- bind_rows(
  ...
  runtime_log_curr |>
    arrange(desc(seconds)) |>
    slice_head(n = min(5, nrow(runtime_log_curr))) |>
...
knitr::kable(runtime_summary, caption = "Runtime summary (total + top 5 steps)")
