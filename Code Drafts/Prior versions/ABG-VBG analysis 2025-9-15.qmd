...
Cutting out ambulatory encounters
...
table2 <- gtsummary::tbl_merge(
  ...
  gtsummary::modify_caption("**Table B. Baseline summary by hypercapnia within ABG and VBG cohorts**")
...
subset_data_vbg <- subset_data %>%
  select(vbg_co2, imv_proc, niv_proc, death_60d, hypercap_resp_failure, w_vbg) %>%
...
weight_model <- weightit(
  has_abg ~ age_at_encounter + sex + factor(race_ethnicity) + curr_bmi + copd + asthma + osa + chf + acute_nmd + phtn + ckd + dm + factor(location) + factor (encounter_type) + temp_new + sbp + dbp + hr + spo2 + sodium + serum_cr + serum_hco3 + serum_cl + serum_lac + serum_k + wbc + plt + bnp + serum_phos + serum_ca,
  data        = subset_data,
  method      = "gbm",
  estimand    = "ATE",
  missing.    = "ind",
...
bal.tab(w_vbg, un = TRUE, m.threshold = 0.1)
...
# Helper to make predictions with standardized x-axis "co2"
mkpred <- function(fit, data_ref, var, group) {
  newd <- data.frame(
    xvar = seq(min(data_ref[[var]], na.rm = TRUE),
               max(data_ref[[var]], na.rm = TRUE),
               length.out = 200)
  )
  names(newd) <- var   # rename back to match the model
  ...
  out$group <- group
  out$co2   <- out[[var]]   # standard x-axis name
  out
}
...
# ABG
pred_imv_abg   <- mkpred(fit_imv,   subset_data_abg, "paco2", "ABG")
pred_niv_abg   <- mkpred(fit_niv,   subset_data_abg, "paco2", "ABG")
pred_death_abg <- mkpred(fit_death, subset_data_abg, "paco2", "ABG")
pred_hcrf_abg  <- mkpred(fit_hcrf,  subset_data_abg, "paco2", "ABG")
...
# -----------------------------
# 1) Extract GBM variable importance
# -----------------------------
extract_gbm_importance <- function(W, top_n = 20) {
  # Robustly find the GBM object inside WeightIt
  gbm_fit <- NULL
  if (!is.null(W$obj)) {
    gbm_fit <- W$obj
  } else if (!is.null(W$info$obj)) {
    gbm_fit <- W$info$obj
  } else if (!is.null(W$info$model.obj)) {
    gbm_fit <- W$info$model.obj
  }
  
  stopifnot(!is.null(gbm_fit), inherits(gbm_fit, "gbm"))
  ...
  imp %>% arrange(desc(rel.inf)) %>% slice_head(n = top_n)
}

# -----------------------------
# 2) Plot GBM importance
# -----------------------------
...
# -----------------------------
# 3) Reconstruct design matrix for SHAP
# -----------------------------
prep_design <- function(W) {
  stopifnot(!is.null(W$covs))
  
  # Convert factors to dummy variables
  X <- cobalt::splitfactor(W$covs, replace = TRUE, drop.levels = TRUE)
  
  # Add missingness indicators + median impute (WeightIt 'missing="ind"')
  na_cols <- names(X)[colSums(is.na(X)) > 0]
  if (length(na_cols)) {
    for (v in na_cols) {
      X[[paste0(v, "_NA")]] <- as.integer(is.na(X[[v]]))
      X[[v]][is.na(X[[v]])] <- stats::median(X[[v]], na.rm = TRUE)
    }
  }
  
  X <- as.data.frame(X)
  
  # Find GBM object robustly
  gbm_fit <- NULL
  if (!is.null(W$obj)) {
    gbm_fit <- W$obj
  } else if (!is.null(W$info$obj)) {
  ...
  # Align columns to GBM model
  X <- X[, intersect(colnames(X), gbm_fit$var.names), drop = FALSE]
  X <- X[, gbm_fit$var.names, drop = FALSE]
...
# -----------------------------
compute_shap <- function(W, nsim = 100, frac_rows = 1.0) {
  mats <- prep_design(W)
  X <- mats$X; gbm_fit <- mats$gbm_fit; best_tree <- mats$best_tree
  
  if (frac_rows < 1.0) {
    set.seed(1)
    X <- X[sample(nrow(X), max(2000, round(frac_rows * nrow(X)))), , drop = FALSE]
  }
  
  pfun <- function(object, newdata) {
    predict(object, newdata = newdata, n.trees = best_tree, type = "response")
  }
  
  set.seed(123)
  shap <- fastshap::explain(gbm_fit, X = X, pred_wrapper = pfun, nsim = nsim, adjust = TRUE)
  list(shap = shap, X = X)
}

# -----------------------------
# 5) Plot SHAP importance
# -----------------------------
plot_shap_importance <- function(shap_list, top_n = 20, title = "SHAP global importance") {
  shap <- shap_list$shap
  imp <- data.frame(
    feature = colnames(shap),
    mean_abs_shap = colMeans(abs(as.matrix(shap)), na.rm = TRUE)
  ) %>% arrange(desc(mean_abs_shap)) %>% slice_head(n = top_n)
  
  ggplot(imp, aes(x = mean_abs_shap, y = reorder(feature, mean_abs_shap))) +
    geom_col() +
    labs(x = "Mean |SHAP| (contribution to logit‑PS scale)", y = NULL, title = title) +
    theme_minimal(base_size = 11)
}
...
```{r}
# -----------------------------
# 1) Extract GBM variable importance for ABG
# -----------------------------
imp_abg <- extract_gbm_importance(w_abg, top_n = 25)  # top 25 features
print(imp_abg)  # shows the relative influence table in the console

# -----------------------------
# 2) Plot GBM importance
# -----------------------------
plot_gbm_importance(imp_abg, title = "ABG selection model – variable importance")

# -----------------------------
# 3) Compute SHAP values
# -----------------------------
# nsim = number of Monte Carlo repetitions, frac_rows = subsample of rows for speed
sh_abg <- compute_shap(w_abg, nsim = 100, frac_rows = 0.5)  # subsample 50% of rows

# Optional: inspect the SHAP values
head(sh_abg$shap)

# -----------------------------
# 4) Plot SHAP importance
# -----------------------------
plot_shap_importance(sh_abg, top_n = 25, title = "ABG selection model – SHAP importance")
