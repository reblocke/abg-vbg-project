...
# 2.  Diagnostics ------------------------------------------------------
library(cobalt)
bal.tab(w, thresholds = c(m = .05))       # SMD target 0.05
love.plot(w)

# Positivity check
histogram(~ w$ps | dat$treat)             # lattice or ggplot2

# 3.  Outcome analysis -------------------------------------------------
library(survey)
dsgn <- svydesign(ids = ~1, weights = ~w$weights, data = dat)
fit  <- svyglm(outcome ~ treat, family = quasibinomial(), design = dsgn)
summary(fit)
